...
class AuthManager extends AbstractManager {
  ...
  static async findByPseudo(pseudo) {
    return new Promise((resolve, reject) => {
      const query = `
      SELECT * FROM Gamer WHERE pseudo = ?
      `;
  ...
  static async registerUser({ pseudo, email, password }) {
    ...
    // Vérification dsi le login est déjà existant
    const existingPseudo = await this.findByPseudo(pseudo);
    ...
    try {
      const result = await new Promise((resolve, reject) => {
        ...
        db.run(
          query,
          [pseudo, email, hashedPassword, defaultRole.id_role, avatar],
          function (err) {
            if (err) {
              reject(err);
            } else {
