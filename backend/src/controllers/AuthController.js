...
const login = async (req, res) => {
  try {
    const { IdUtilisateur, Email } = await models.auth.authenticateUser(req.body);
    if (!IdUtilisateur) {
      res.status(401).json({ success: false, message: 'Aucun utilisateur trouvé avec ces informations.' });
    } else {
